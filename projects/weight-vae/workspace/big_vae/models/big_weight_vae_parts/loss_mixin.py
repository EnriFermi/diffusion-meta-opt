...
class BigWeightVAELossMixin:
    ...
    def patch_structure_loss(
        ...
        if d_in_mask is not None:
            ...
            d_in_mask_bool = d_in_mask.to(device=W.device, dtype=torch.bool)
        else:
            d_in_mask_bool = torch.ones((B, d_in), device=W.device, dtype=torch.bool)

        # A final partial patch is still a real part of the weight matrix.  The
        # previous floor division silently dropped it (for example 11/27 input
        # coordinates of the first ResNet convolution), which becomes fatal
        # coverage loss when structural supervision is used on its own.
        T = (d_in + p - 1) // p
        ...
        used = T * p
        pad_d_in = used - d_in
        W_padded = F.pad(W, (0, 0, 0, pad_d_in)) if pad_d_in else W
        W_hat_padded = F.pad(W_hat, (0, 0, 0, pad_d_in)) if pad_d_in else W_hat
        d_in_component_mask = (
            F.pad(d_in_mask_bool, (0, pad_d_in), value=False) if pad_d_in else d_in_mask_bool
        ).view(B, T, p)

        X = W_padded.transpose(1, 2).contiguous().view(B, d_out, T, p)
        X_hat = W_hat_padded.transpose(1, 2).contiguous().view(B, d_out, T, p)

        N = B * d_out
        X = X.reshape(N, T, p)
        X_hat = X_hat.reshape(N, T, p)
        component_mask_flat = (
            d_in_component_mask.unsqueeze(1)
            .expand(B, d_out, T, p)
            .reshape(N, T, p)
        ...
        if d_out_mask_bool is not None:
            ...
            component_mask_flat = component_mask_flat * d_out_mask_flat.unsqueeze(-1)

        X = X * component_mask_flat
        X_hat = X_hat * component_mask_flat
        element_mask_flat = (component_mask_flat.sum(dim=-1) > 0).to(dtype=W.dtype)

        def _valid_mean_per_sample(values: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
            if tuple(values.shape) != tuple(mask.shape):
                raise ValueError(
                    f"masked structural reduction shape mismatch: {tuple(values.shape)} vs {tuple(mask.shape)}"
                )
            flat_values = values.reshape(B, -1)
            flat_mask = mask.reshape(B, -1).to(device=values.device, dtype=values.dtype)
            numerator = (flat_values * flat_mask).sum(dim=1)
            denominator = flat_mask.sum(dim=1).clamp_min(1.0)
        ...
        if use_dir:
            if pred_dirs is not None:
                u_hat_dir = pred_dirs[:, :, :T, :].reshape(N, T, p) * component_mask_flat
                u_hat_dir = u_hat_dir / (u_hat_dir.norm(dim=-1, keepdim=True) + eps)
            else:
            ...
            dir_weight = dir_weight * element_mask_flat
            dir_weight = dir_weight / dir_weight.sum(dim=1, keepdim=True).clamp_min(1.0)
            per_output_dir = (dir_loss * dir_weight).sum(dim=1)
            valid_outputs = (element_mask_flat.sum(dim=1) > 0).to(dtype=per_output_dir.dtype)
            L_dir = _valid_mean_per_sample(per_output_dir, valid_outputs)
        ...
        if use_scale:
            ...
            L_scale = _valid_mean_per_sample(huber, element_mask_flat)

        if use_rec:
            rec_num = (X_hat - X).pow(2).sum(dim=-1)
            rec_den = r.pow(2) + eps
            rec_loss = rec_num / rec_den
            L_rec = _valid_mean_per_sample(rec_loss, element_mask_flat)
        ...
        if use_rel:
            ...
            pair_mask = element_mask_flat.unsqueeze(1) * element_mask_flat.unsqueeze(2)
            L_rel = _valid_mean_per_sample(rel_sq, pair_mask)
    ...
    @staticmethod
    def operator_relative_mse_loss(
        X: torch.Tensor,
        W: torch.Tensor,
        W_hat: torch.Tensor,
        x_mask: torch.Tensor | None = None,
        ...
        """Dimensionless per-operator error in activation space.

        This is the empirical activation-covariance metric
        ``||X @ (W_hat - W)||^2 / ||X @ W||^2``.  The ratio is reduced per
        operator before the batch mean so high-energy operators do not set the
        task weighting for low-energy operators.
        """
        squeeze_batch = False
        if X.ndim == 2:
            if W.ndim != 2 or W_hat.ndim != 2:
                raise ValueError(
                    "unbatched operator_relative_mse_loss expects X=[n,d_in], "
                    "W=[d_in,d_out], and W_hat=[d_in,d_out]"
                )
            X = X.unsqueeze(0)
            W = W.unsqueeze(0)
            W_hat = W_hat.unsqueeze(0)
        ...
        if X.ndim != 3 or W.ndim != 3 or W_hat.ndim != 3:
            raise ValueError(
                "operator_relative_mse_loss expects X=[B,n,d_in], "
                "W=[B,d_in,d_out], and W_hat=[B,d_in,d_out]"
            )
        if tuple(W.shape) != tuple(W_hat.shape):
            raise ValueError(f"W/W_hat shape mismatch: {tuple(W.shape)} vs {tuple(W_hat.shape)}")
        batch, samples, d_in = X.shape
        if tuple(W.shape[:2]) != (batch, d_in):
            raise ValueError(f"X/W shape mismatch: X={tuple(X.shape)} W={tuple(W.shape)}")

        if d_in_mask is not None:
            if tuple(d_in_mask.shape) != (batch, d_in):
                raise ValueError(
                    f"d_in_mask must be {(batch, d_in)}, got {tuple(d_in_mask.shape)}"
                )
            X = X * d_in_mask.to(device=X.device, dtype=X.dtype).unsqueeze(1)
        prediction = torch.matmul(X, W_hat)
        target = torch.matmul(X, W)

        value_mask = torch.ones(
            (batch, samples, int(W.shape[-1])),
        ...
        if x_mask is not None:
            if tuple(x_mask.shape) != (batch, samples):
                raise ValueError(
                    f"x_mask must be {(batch, samples)}, got {tuple(x_mask.shape)}"
                )
            value_mask = value_mask * x_mask.to(
                device=prediction.device, dtype=prediction.dtype
            ).unsqueeze(-1)
        if d_out_mask is not None:
            expected = (batch, int(W.shape[-1]))
            if tuple(d_out_mask.shape) != expected:
                raise ValueError(
                    f"d_out_mask must be {expected}, got {tuple(d_out_mask.shape)}"
                )
            value_mask = value_mask * d_out_mask.to(
                device=prediction.device, dtype=prediction.dtype
            ).unsqueeze(1)

        error_energy = ((prediction - target).float().square() * value_mask).sum(
            dim=(1, 2)
        )
        target_energy = (target.float().square() * value_mask).sum(dim=(1, 2))
        relative_mse = error_energy / target_energy.clamp_min(float(eps))
        result = relative_mse.mean()
        if squeeze_batch:
