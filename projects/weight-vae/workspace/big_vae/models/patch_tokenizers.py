...
from big_vae.models.vae_shared import NonAffineRMSNorm
...
class AlignedConditionedPatchBlock(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.base_return_norm = NonAffineRMSNorm(d_model) if self.peri_rms_residual else nn.Identity()
        self.condition_return_norm = NonAffineRMSNorm(d_model) if self.peri_rms_residual else nn.Identity()
        ...
        if self.single_gate_v3:
            ...
            nn.init.constant_(self.gate_fc.bias, math.log(0.2 / 0.8))
        else:
    ...
    def forward(self, u: torch.Tensor, x_c: torch.Tensor) -> torch.Tensor:
        ...
        if self.peri_rms_residual:
            base = self.base_return_norm(base) * self.residual_scale
        ...
        if self.peri_rms_residual:
            delta = self.condition_return_norm(delta) * self.residual_scale
        gate = torch.sigmoid(self.gate_fc(inter))
        if self.single_gate_v3:
            return u + base + gate * delta
...
class ResidualPatchTokenizer(nn.Module):
    ...
    def __init__(
        ...
        self.peri_rms_residual = bool(peri_rms_residual)
        self.return_norm = NonAffineRMSNorm(self.d_patch) if self.peri_rms_residual else nn.Identity()
...
class PatchConditionedMLPTokenizer(nn.Module):
    ...
    def __init__(
        ...
        residual_scale = 1.0 / math.sqrt(float(2 * block_count)) if peri_rms_residual else 1.0
