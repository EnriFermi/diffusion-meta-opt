...
class BackgroundPrefetcher(Generic[T]):
    ...
    def wait_closed(self, timeout_s: float | None = None) -> bool:
        """Wait for producer termination and report whether it actually stopped.

        ``close`` retains its historical one-second best-effort behavior for
        normal training. Callers that snapshot producer-owned mutable state use
        this explicit fail-closed barrier after requesting shutdown.
        """

        if timeout_s is not None and timeout_s < 0:
            raise ValueError(f"timeout_s must be non-negative or None, got {timeout_s}")
        if not self._started:
            return True
