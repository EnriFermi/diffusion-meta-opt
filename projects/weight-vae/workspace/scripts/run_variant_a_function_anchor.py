...
DEFAULT_SUMMARY = (
    ROOT
    / "docs/reparam_preconditioning_experiments/variant_A_causal_debug/function_anchor_runs/function_anchor_summary.csv"
)


RUN_SPECS: tuple[dict[str, float | str], ...] = (
    {
        "suffix": "control_anchor_c0p0010",
        "run_label": "sage_cnn_vae_smoothing_celo_meta_control_logit_anchor_c0p0010_v1_seed0",
        "precond_kind": "none",
        "alpha": 0.0,
        "cap": 0.0,
        "anchor_coeff": 0.0010,
    ...
    {
        "suffix": "a_cap1_anchor_c0p0010",
        "run_label": "sage_cnn_vae_smoothing_celo_meta_li_a_hvp_local_cap1_logit_anchor_c0p0010_v1_seed0",
...
def _selected_specs(names: set[str] | None) -> tuple[dict[str, float | str], ...]:
    ...
    if missing:
        raise ValueError(f"unknown function-anchor spec suffixes: {missing}")
    return result
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run matched Variant A/control with a deterministic logit function anchor.")
    ...
    print(
        "[variant_a_function_anchor] start "
    ...
    for idx, spec in enumerate(specs, start=1):
        ...
        print(
            "[variant_a_function_anchor] run "
            f"{idx}/{len(specs)} suffix={suffix} precond={precond_kind} alpha={alpha:.6g} cap={cap:.6g} "
            f"anchor_coeff={anchor_coeff:.6g} run_label={run_label} output_dir={output_dir} config_hash={config_hash(cfg)}",
            flush=True,
        )
        print(f"[variant_a_function_anchor] resolved_config={json.dumps(asdict(cfg), sort_keys=True, default=str)}", flush=True)
        ...
        print(
            "[variant_a_function_anchor] summary "
            ...
            f"anchor_ce_delta_final={row.get('train_function_anchor_ce_delta_final', float('nan')):.6g} "
            f"summary_out={args.summary_out}",
            flush=True,
        )
    elapsed = time.perf_counter() - t0
    print(f"[variant_a_function_anchor] done elapsed_sec={elapsed:.2f} summary_out={args.summary_out}", flush=True)
