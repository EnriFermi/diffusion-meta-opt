...
def build_big_vae_model_config(raw_cfg: Mapping[str, Any]) -> ModelConfig:
    ...
    return ModelConfig(
        ...
        big_vae=BigVAEConfig(
            architecture_version=str(big_cfg.get("architecture_version", "legacy_v1")),
            ...
            # Checkpoints produced before ``rope_2d_coord_kind`` was added used
            # raw arange coordinates.  Defaulting a missing serialized field to
            # ``normalized_center`` keeps the state-dict shapes loadable while
            # silently changing the learned function.  Current training configs
            # specify ``normalized_center`` explicitly, so ``raw`` is the safe
            # backward-compatible fallback for legacy checkpoint payloads.
            rope_2d_coord_kind=str(big_cfg.get("rope_2d_coord_kind", "raw")),
            ...
            weight_input_normalization_kind=str(
                big_cfg.get("weight_input_normalization_kind", "none")
            ),
            weight_input_scale_qmax=float(big_cfg.get("weight_input_scale_qmax", 7.0)),
            weight_input_log2_scale_mean=float(
                big_cfg.get("weight_input_log2_scale_mean", 0.0)
            ),
            weight_input_log2_scale_std=float(
                big_cfg.get("weight_input_log2_scale_std", 1.0)
            ),
            latent_feedback_start_layer=int(big_cfg.get("latent_feedback_start_layer", 1)),
            latent_feedback_attn_dim=int(big_cfg.get("latent_feedback_attn_dim", 512)),
            latent_feedback_n_heads=int(big_cfg.get("latent_feedback_n_heads", 16)),
            decoder_bridge_attn_dim=int(big_cfg.get("decoder_bridge_attn_dim", 512)),
            decoder_bridge_n_heads=int(big_cfg.get("decoder_bridge_n_heads", 8)),
            v9_refinement_blocks=int(big_cfg.get("v9_refinement_blocks", 40)),
            v9_router_width=int(big_cfg.get("v9_router_width", 384)),
            v9_router_ffn_width=int(big_cfg.get("v9_router_ffn_width", 1536)),
            v9_score_cap=float(big_cfg.get("v9_score_cap", 0.1)),
            v9_residual_scale=float(big_cfg.get("v9_residual_scale", 0.11180339887498948)),
            v9_anchor_sigma_keys=float(big_cfg.get("v9_anchor_sigma_keys", 0.6)),
            v9_bypass_refinement=bool(big_cfg.get("v9_bypass_refinement", False)),
            v9a_carrier_mix=float(big_cfg.get("v9a_carrier_mix", 0.1)),
            v9a_protected_anchor_floor=float(
                big_cfg.get("v9a_protected_anchor_floor", 0.25)
            ),
            v9a_aggregation=str(big_cfg.get("v9a_aggregation", "sqrt_depth_sum")),
            v10_protected_dim=int(big_cfg.get("v10_protected_dim", 320)),
            v10_adaptive_dim=int(big_cfg.get("v10_adaptive_dim", 64)),
            v10_num_experts=int(big_cfg.get("v10_num_experts", 40)),
            v11_num_trunks=int(big_cfg.get("v11_num_trunks", 4)),
            v11_blocks_per_trunk=int(big_cfg.get("v11_blocks_per_trunk", 10)),
            v11_trunk_dim=int(big_cfg.get("v11_trunk_dim", 16)),
