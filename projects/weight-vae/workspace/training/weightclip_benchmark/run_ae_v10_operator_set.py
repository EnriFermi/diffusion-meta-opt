...
SCHEMA = "weightclip_ae_v10_exact64_orthogonal_complement_v2"
EXPECTED_TOTAL_PARAMETERS = 718_886_017
EXPECTED_TRAINABLE_PARAMETERS = 236_068_480
EXPECTED_ACTIVE_ENCODER_PARAMETERS = 69_483_904


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the fresh exact64 V10 orthogonal-complement diagnostic."
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("conf/weightclip_benchmark/ae_v10_exact64_orthogonal_complement_1984step.yaml"),
    )
...
def _load_spec(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict) or set(payload) != expected:
        raise ValueError("V10 exact64 config keys do not match the frozen schema")
    if payload["schema"] != SCHEMA:
        raise ValueError(f"V10 exact64 schema must be {SCHEMA!r}")
    frozen = {
        "operator_count": 64,
        "steps": 1984,
        "physical_batch": 6,
    ...
    for key, value in frozen.items():
        if int(payload[key]) != value:
            raise ValueError(f"V10 exact64 requires {key}={value}")
    if str(payload["layer_key"]) != "layer3.0.conv2.weight":
        raise ValueError("V10 exact64 fixes layer3.0.conv2.weight")
    return payload
...
def _build_worker_config(
    ...
    train["telemetry"]["grad_layer_monitor"].update(
        {
            ...
            "include_prefixes": [
                ...
                "orthogonal_complement_encoder_v10",
    ...
    if (
        ...
        raise AssertionError("V10 exact64 must preserve the frozen structural-only objective")
...
def main() -> None:
    ...
    summary = {
        "schema": SCHEMA,
        "architecture": "orthogonal_complement_tied_posfilm_v10",
        ...
        "physical_batch": 6,
        "grad_accum_steps": 3,
        "latent_contract": "[B,32,384]=zc[320]+za[64] only",
        ...
        "parameter_contract": {
            ...
            "v10_experts": 64_199_680,
            "per_expert": 1_604_992,
        },
        "selection": manifest,
    }
    print("[v10-exact64] stage=preflight", flush=True)
    print(json.dumps(summary, indent=2, sort_keys=True), flush=True)
    if args.dry_run:
        print("[v10-exact64] stage=dry-run-complete no_files_written=true", flush=True)
        return
    ...
    print(f"[v10-exact64] stage=train resolved={resolved_path} selection={selection_path}", flush=True)
    _run_worker(
        rank=0,
        world_size=1,
        cfg_dict=cfg,
    ...
    if not expected.is_file() or (checkpoint_root / "latest.pt").exists():
        raise RuntimeError("V10 exact64 final-only checkpoint contract failed")
    print(f"[v10-exact64] stage=complete checkpoint={expected}", flush=True)
