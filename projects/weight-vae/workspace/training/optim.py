...
_NO_WEIGHT_DECAY_PARAMETER_NAMES = {
    "cls_token",
    "latent_base",
    "vamp_prior_base",
}
...
def _build_matrix_decay_param_groups(
    model: torch.nn.Module,
    *,
    lr: float,
    weight_decay: float,
) -> list[dict[str, Any]]:
    """Apply AdamW decay to learned matrices, not norms/biases/latent tokens."""
    ...
    for raw_name, parameter in model.named_parameters():
        if not parameter.requires_grad:
            continue
        name = _strip_optimizer_wrapper_prefixes(raw_name)
        leaf_name = name.rsplit(".", 1)[-1]
        if parameter.ndim < 2 or leaf_name in _NO_WEIGHT_DECAY_PARAMETER_NAMES:
...
def build_adamw_optimizer(
    ...
    *,
    ...
    weight_decay_2d_only = bool(train_cfg.get("weight_decay_2d_only", False))
    ...
    if weight_decay_2d_only:
        if patch_tokenizer_alpha_lr > 0.0:
            raise ValueError(
                f"{section}.weight_decay_2d_only cannot be combined with patch_tokenizer_alpha_lr"
            )
        optimizer_params = _build_matrix_decay_param_groups(
            model,
            lr=lr,
            weight_decay=weight_decay,
        )
...
def build_cosine_scheduler(
    ...
    *,
    ...
    if scheduler_name not in {"cosine", "cosine_decay"}:
        raise ValueError(
            f"Unsupported {section}.scheduler_name={scheduler_name!r}. "
            "Expected one of: 'cosine', 'constant', 'none'"
        )
